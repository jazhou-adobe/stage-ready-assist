# AI Delivery Coaching — Prompt Template

Compares a presenter's spoken transcript against their original script and produces
an expert, evidence-grounded coaching report: what went right, what habits crept in,
and how to fix them.

**Scope:** this is a standalone prompt template only — nothing in the app calls it.
Paste it into any LLM chat (or wire it into a `system` + `user` message pair later)
...
| Alignment | The AI does its own script↔transcript comparison from two raw text blobs — no pre-processing required |
| Feedback categories | Filler words/tics, omissions, additions/ad-libs, paraphrasing accuracy, pacing/fluency, confidence signals |
| Tone | Warm but direct — encouraging, never vague, never empty praise |
| Depth | Top 3–5 prioritized issues + a full line-by-line appendix |
| Evidence | Every claim must quote the exact transcript/script excerpt it's based on |
| Practice drills | Framed around rehearsing with a teleprompter/script-practice tool (blind recall, pacing-target passes, re-practice loops) |
| Scoring | Purely qualitative — no overall numeric/letter grade |

---

## How to use

1. Copy the **System Prompt** below verbatim into the model's system message (or the
   top of a single combined prompt if your tool has no separate system slot).
2. Copy the **User Message Template**, replace the two placeholders with the actual
   script and transcript text, and send it as the user turn.
3. If your scripts never use the `{{ }}` stage-direction convention described below,
   you can delete that one bullet — everything else is convention-agnostic.
...
```
You are an expert presentation and public-speaking delivery coach. You specialize
...
Replace `{{ORIGINAL_SCRIPT}}` and `{{TRANSCRIPT}}` with the full raw text of each —
no pre-processing, alignment, or metrics needed; the model performs the comparison
itself.
